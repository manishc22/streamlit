...
def get_store_data():
    data = supabase.table('store_audits').select(
        'id', 'created_at', 'image1_id', 'store_name').eq('image1_audited', 'FALSE').execute()
    return data


st.write("### Dealerboard Audits")
st.divider()
...
image = (df_alerts1['image1_id']).values[counter]
    ...
    with st.form("Dealerboard Audit", clear_on_submit=True):
        ...
        if submitted:
            ...
            data_insert = supabase.table('dealerboard').insert(
                {'image_correct': image_correct, 'selfie_dealerboard': selfie, 'month': month,
                 'cycle': cycle}).execute()
